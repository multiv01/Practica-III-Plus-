...
AddEventHandler('esx_hackowanie:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_hackowanie:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('esx_hackowanie:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('esx_hackowanie:robberyStarted')
AddEventHandler('esx_hackowanie:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.PoliceNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('esx_hackowanie:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('esx_hackowanie:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('esx_hackowanie:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('esx_hackowanie:robberyComplete', _source, store.reward)
